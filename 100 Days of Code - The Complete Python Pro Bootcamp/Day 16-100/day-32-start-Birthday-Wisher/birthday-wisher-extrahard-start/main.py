...
# Had to use ChatGPT to get me the mask working. I had not yet come acorss it. Maybe the explainer video will have
...
mask = (
        (birthday_list["month"] == today_month) &
        (birthday_list["day"] == today_day)
    )
today_row = birthday_list[mask]
name = today_row["name"].iloc[0]
...
with open(f"./letter_templates/{letter}", "r") as letter:
    body = letter.read()
    new_letter = body.replace("[NAME]", f"{name}")
